...
class _Parser:
    ...
    def expr(self):
        return self._expr(self.take_left())
    ...
    def _expr(self, tok: str):
        ...
        # 遇到起始符号返回rest
            ...
            return self._rest(self.get_left())

        # 遇到引用比如【('Some (+ 1 2))】则构建一个Pair, 并重新对自身再次求表达式
        elif be_quote(tok):
            return Pair("quote", Pair(self._expr(self.take_left()), nil))
    ...
    def _rest(self, token: str):
        try:
            ...
            return Pair(self._expr(self.take_left()), self._rest(self.get_left()))
        except IndexError:
